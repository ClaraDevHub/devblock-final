...
export function App() {
  ...
  // GET: Buscar dados
  useEffect(() => {
    // IMPORTANTE: Altere para a URL da sua API hospedada se for fazer deploy real
  ...
  // useMemo para ordenar (Pendentes primeiro)
      ...
      <main className="w-full max-w-2xl bg-white p-6 rounded-lg shadow-md">
        ...
        <section className="mb-8 bg-gray-50 p-4 rounded-md border border-gray-200">
          ...
          <form onSubmit={handleCreateBlock} className="flex flex-col gap-4">
            <div className="flex flex-col">
              <label htmlFor="title" className="text-sm font-medium text-gray-700 mb-1">Título</label>
              <input 
                id="title" 
                ref={titleInputRef} 
                type="text" 
                ...
                placeholder="Ex: Erro de CORS no Axios" 
            ...
            <div className="flex flex-col">
              ...
              <textarea 
                ...
                className="border rounded-md p-2 focus:ring-2 focus:ring-blue-500 outline-none resize-none h-20" 
              />
            </div>
            <div className="flex flex-col">
              <label htmlFor="url" className="text-sm font-medium text-gray-700 mb-1">Link (Opcional)</label>
              <input 
                id="url" 
                type="url" 
                value={classUrl} 
            ...
            <button type="submit" className="bg-blue-600 text-white py-2 rounded-md font-semibold hover:bg-blue-700 transition-colors">
              Adicionar Bloqueio
            </button>
          </form>
        </section>
        ...
        <section>
          ...
          {isLoading ? (
            ...
            <p className="text-gray-500 text-center py-4">Nenhum bloqueio registrado. 🚀</p>
          ) : (
            <ul className="space-y-4">
              {sortedBlocks.map(block => (
                <li key={block.id} className={`p-4 border rounded-md flex flex-col gap-3 ${block.status === 'resolved' ? 'bg-gray-50 opacity-75' : 'bg-white'}`}>
                  ...
                  <div className="flex gap-2 mt-2 pt-2 border-t">
                    {block.status === 'pending' && (
                      <button onClick={() => handleResolveBlock(block.id)} className="text-sm bg-green-50 text-green-600 px-3 py-1 rounded hover:bg-green-100 font-medium">✓ Resolver</button>
                    )}
                    <button onClick={() => handleDeleteBlock(block.id)} className="text-sm bg-red-50 text-red-600 px-3 py-1 rounded hover:bg-red-100 font-medium">🗑️ Excluir</button>
                  </div>
                </li>
              ))}
            </ul>
